...
class WP_Optimization_images extends WP_Optimization {
	...
	private $_attachments_meta_data;
	...
	public function get_last_scan_time($key) {
		$time = $this->options->get_option('unused_images_last_scan_'.$key);
	...
	public function output_csv() {
		...
		foreach ($this->blogs_ids as $blog_id) {
			...
			$prepare_args = array_merge(array($meta_id, '_wp_attachment_metadata'), $unused_posts_images, array($limit));
			
			while ($offset < $total_images) {
				global $wpdb;
				// phpcs:disable
				// WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- false positive
				// WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- false positive
				...
				if (!empty($all_image_sizes)) {
					foreach ($all_image_sizes as $image_size => $info) {
						...
						if (array_key_exists($image_size, $image_sizes)) {
							$image_sizes[$image_size]['used'] = $used ?: $image_sizes[$image_size]['used'];
		...
		if ($return_images) {
			...
			if ($params['blog_id'] > 0) {
				foreach (array_keys($unused_images) as $blog_id) {
					if ((int) $params['blog_id'] !== (int) $blog_id) unset($unused_images[$blog_id]);
				}
			}
	...
	public function task_get_posts_images($offset = 0, $limit = 500, $blog_id = 1) {
		...
		// when image was not found in the Media Library we get image filename in the result
	...
	private function get_posts_content_images(&$posts, $blog_id) {
		...
		foreach ($posts as $post) {
			...
			if (!empty($images)) {
				foreach ($images as $image) {
					...
					// this cause the issue with detecting used/unused images that is why we add information about filename found in the post and later check both filenames in get_image_attachment_id_bulk().
		...
		if (!empty($found_images)) {
			...
			return array_unique(array_merge($post_content_images, $plugin_images_from_metadata, $plugin_images));
		} else {
	...
	private function get_oxygen_images() {
		...
		do {
			...
			foreach ($posts as $post) {
				// We need wp_unslash() here because Oxygen 6 stores meta_value as JSON-encoded data.
				// Using wp_unslash() ensures we retrieve the correct values.
				$images = $this->parse_images_in_content(wp_unslash($post->meta_value));

				$found_images_ids = array_merge($found_images_ids, $this->parse_oxygen_images_ids_in_content($post->meta_value));
		...
		$result = $this->get_images_ids_files($found_images);
		if (!empty($found_images_ids)) {
			$result['ids'] = array_unique(array_merge($result['ids'], $found_images_ids));
		}
	...
	/**
	 * Get image IDs from a string for Oxygen galleries
	 *
	 * @param string $content
	 * @return array
	 */
	private function parse_oxygen_images_ids_in_content($content) {
		$image_ids = array();

		preg_match_all('/"image_ids":"([\d,]+)"/', $content, $matches);
	
		if (!empty($matches[1])) {
			foreach ($matches[1] as $match) {
				$image_ids = array_merge($image_ids, explode(',', $match));
			}
		}
	...
	/**
	 * Scan post meta values for Revslider slides images.
	 *
	 * @return array
	 */
	private function get_revslider_slides() {
		...
		$db_tables = WP_Optimize()->get_db_info()->get_show_full_tables();
		$db_table_name_slides = $wpdb->prefix . 'revslider_slides';
		$db_table_name_revslider_sliders7 = $wpdb->prefix . 'revslider_sliders7';

		$sql = '';

		// Get SQL for older versions
		if (array_key_exists($db_table_name_slides, $db_tables)) {
			$sql .= "SELECT params, layers FROM {$db_table_name_slides}";
		}

		// Get SQL for versions 6.7 and above
		if (array_key_exists($db_table_name_revslider_sliders7, $db_tables)) {
			if ('' !== $sql) $sql .= ' UNION ';
			$sql .= "SELECT params, NULL AS layers FROM {$db_table_name_revslider_sliders7}";
		}

		// edge case, no known database tables exist
		if ('' === $sql) {
			return array(
				'ids' => array(),
				'files' => array(),
		...
		do {
			$records = $wpdb->get_results("{$sql} LIMIT {$offset}, {$limit};"); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- We don't need to prepare it, as all variables are hardcoded, literal strings

			foreach ($records as $item) {
				// The slide's background image is stored in 'params'
				$params = json_decode($item->params);
				if (!empty($params) && is_object($params)) {
					$this->extract_revslider_background_images($params, $found_images, $found_files);
					$this->extract_revslider_images($params, $found_images, $found_files);
				}

				$this->extract_revslider_layer_images((string) $item->layers, $found_images);
			}
	...
	/**
	 * Extract information about Revslider background images
	 *
	 * @param object|null $params       params field from revslider database table
	 * @param array $found_images
	 * @param array $found_files
	 * @return void
	 */
	private function extract_revslider_background_images(?object $params, array &$found_images, array &$found_files) {
		if (isset($params->bg) && isset($params->bg->type) && 'image' === $params->bg->type) {
	...
	/**
	 * Extract information about images from revslider images data (in version 6.7 and newer)
	 *
	 * @param object|null $params       params field from revslider database table
	 * @param array $found_images images ids
	 * @param array $found_files  images paths
	 * @return void
	 */
	private function extract_revslider_images(?object $params, array &$found_images, array &$found_files) {
		if (isset($params->imgs) && is_array($params->imgs)) {
			foreach ($params->imgs as $image) {
				if (!empty($image->lib_id)) {
					$found_images[] = $image->lib_id;
				} elseif (!empty($image->src)) {
					$base_upload_url = $this->get_upload_base_url();
					$image_record_value = str_replace($base_upload_url.'/', '', $image->src);
					$image_id = $this->get_image_attachment_id($image_record_value);
	...
	/**
	 * Extract information about images from revslider layers data (in versions prior to 6.7)
	 *
	 * @param string $layers       field from revslider database table
	 * @param array $found_images images ids
	 * @return void
	 */
	private function extract_revslider_layer_images(string $layers, array &$found_images) {
	...
	private function get_single_image_ids_in_post_meta() {
		...
		// phpcs:disable
		// WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- false positive
		// WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- false positive
	...
	public function process_get_posts_images_result($result) {
		...
		if ($result['processed'] < $result['limit']) {
			...
			// get Oxygen images.
			if (defined('CT_VERSION') || (defined('BREAKDANCE_MODE') && 'oxygen' === BREAKDANCE_MODE)) {
				$found_images_ids_files = $this->get_oxygen_images();
				$found_images_ids = array_merge($found_images_ids, $found_images_ids_files['ids']);
				$found_images_files = array_merge($found_images_files, $found_images_ids_files['files']);
			}
			// Slider revolution images
			if (class_exists('RevSliderFront')) {
				$found_images_ids_files = $this->get_revslider_slides();
				$found_images_ids = array_merge($found_images_ids, $found_images_ids_files['ids']);
				$found_images_files = array_merge($found_images_files, $found_images_ids_files['files']);
			}

			// add homepage images ids.
			$found_images_ids_files = array_merge($found_images_ids, $this->get_homepage_images($blog_id));
			$found_images_ids = array_merge($found_images_ids, $found_images_ids_files['ids']);
			$found_images_files = array_merge($found_images_files, $found_images_ids_files['files']);
	...
	/**
	 * Scans a subdirectory and returns image files which are not associated to any media record.
	 *
	 * @param string $sub_directory upload subdirectory.
	 * @param int    $blog_id
	 * @return array
	 */
		...
		if ($handle = opendir($sub_directory_path)) {
			...
			while (false !== $file) {
				...
				if (false === $file || (($current_query_length + $static_query_length) >= $max_packet_size) || ($free_memory < $keep_free_memory && count($_image_files) > $min_images_per_check)) {
					...
					// walk through found image files and check if their relation in database found.
					...
					if (array_key_exists($blog_id, $images)) {
						...
						foreach ($images[$blog_id] as $image) {
							if (is_numeric($image)) {
								...
								$removed['files']++;
								$removed[$blog_id]++;
								$removed['size'] += $unused_images_files[$image] ?? 0;
			...
			foreach ($this->blogs_ids as $blog_id) {
				...
				if (!empty($ids)) {
					foreach ($ids as $id) {
						...
						// if metadata found for attachment then check resized images.
	...
	private function get_host() {
		...
		if ($this->is_multisite_mode()) {
			...
			// If same blog ID and host is set, return cached host
			if ($current_blog_id === $blog_id && !empty($host)) {
				return $host;
			}

			// Update blog ID when switching sites
			$blog_id = $current_blog_id;
		} else {
			if (!empty($host)) return $host;
		}
		
		$parsed_url = wp_parse_url($this->get_upload_base_url());
	 ...
	 * @param  bool   $return_nonexistent if true then non-existent attachments will return too with id = false.
	...
	public function get_image_attachment_id_bulk($filenames, $return_nonexistent = false) {
		...
		// walk through $filenames and check if there is any with resized filename
		...
		if ($return_nonexistent) {
			// fill non-existing filenames with false.
			foreach ($filenames as $filename) {
	...
	public function get_original_image_file_name($filename) {
		if (preg_match($this->image_filename_regexp, $filename, $parts)) {
			$relative_path = $parts[1] . $parts[5];
		
			if ($this->is_multisite_mode() && get_current_blog_id() > 1) {
				// Remove 'sites/<id>/' only if present at the beginning
				$relative_path = preg_replace('#^sites/\d+/#', '', $relative_path);
			}
	...
	private function clear_cached_data() {
		...
		// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Safe, no user input used
	 ...
	 * @return array
	 */
	private function parse_images_in_content(string $content): array {
		$host = $this->get_host();
		...
		if (!empty($images[0])) {
			if ('' !== $host) {
				foreach ($images[1] as $i => $image_url) {
	...
	/**
	 * Checks if the image url is for local server image
	 *
	...
	private function is_local_image($image_url) {
		$host = $this->get_host();
		return substr($image_url, 0, 2) !== '//' || strpos($image_url, $host) !== false;
	}
	 ...
	 */
	private function preload_attachments_metadata(array &$attachment_ids) {
		global $wpdb;
		...
		if (count($attachment_ids) <= $preload_batch_limit) {
			...
			$placeholder_string = implode(',', $placeholders);
			$prepare_args = array_merge(array('_wp_attachment_metadata'), $attachment_ids);
			...
			$placeholder_string = implode(',', $placeholders);
			$prepare_args = array_merge(array('_wp_attachment_metadata'), $loaded_meta_ids);
			// phpcs:disable
			// WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- false positive
			$metadata = $wpdb->get_results(
	...
	/**
	 * Get the key value for storing attachment metadata
	 *
