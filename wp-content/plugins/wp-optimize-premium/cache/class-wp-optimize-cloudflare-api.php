...
class WP_Optimize_Cloudflare_API {
	...
	public function get_all_zones() {
		...
		while ($page < $total_pages) {
			...
			$total_pages = $zones_page['result_info']['total_pages'] ?? $total_pages;
		}
	 ...
	 * @return array|WP_Error
	 */
	private function do_request(string $method, string $endpoint, array $params = array(), array $query_params = array()) {
	...
	private function debug($info) {
		if (defined('WP_DEBUG') && WP_DEBUG) {
			...
			error_log("WP_Optimize_Cloudflare: $info"); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Using for debugging purpose
		}
	}
}
