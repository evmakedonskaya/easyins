...
class WPO_Cache_Premium {
	...
	public function __construct() {
		...
		add_action('wpo_after_cache_settings', array($this, 'output_rest_cache_option'), 10);

		if (WP_Optimize()->get_page_cache()->is_enabled() && $this->config['enable_rest_caching']) {
			WP_Optimize_REST_Cache::instance();
		}
	...
	public function get_always_purge_post_config($post_id) {
		...
		foreach ($always_purge_config as $item) {
			if ($post_id !== (int) $item['post_id']) continue;
	...
	public function add_cache_query_variables($variables, $config) {
		...
		if (defined('NELIO_AB_TESTING') && class_exists('Nelio_AB_Testing')) {
			$variables[] = 'nab';
		}
	...
	public function maybe_set_tax_country_cookie() {
		...
		$billing_country = TeamUpdraft\WP_Optimize\Includes\Fragments\fetch_superglobal('post', 'billing_country', null, 'sanitize_text_field');
		$country_preselect = TeamUpdraft\WP_Optimize\Includes\Fragments\fetch_superglobal('request', 'wc_country_preselect', null, 'sanitize_text_field');

		if (null !== $billing_country) {
			// Value from WooCommerce form
			setcookie('woocommerce_tax_country', $billing_country, (time() + 30 * 86400), '/');
			$_COOKIE['woocommerce_tax_country'] = $billing_country;
			return;
		} elseif (null !== $country_preselect) {
			// Value from VAT compliance form
			setcookie('woocommerce_tax_country', $country_preselect, (time() + 30 * 86400), '/');
			$_COOKIE['woocommerce_tax_country'] = $country_preselect;
			return;
		}
		...
		$cookie_country = TeamUpdraft\WP_Optimize\Includes\Fragments\fetch_superglobal('cookie', 'woocommerce_tax_country', null, 'sanitize_text_field');

		if (null === $cookie_country) {
	...
	/**
	 * Output REST cache option
	 *
	 * @return void
	 */
	public function output_rest_cache_option() {
		$extract = array(
			'wpo_cache_options' => $this->config,
		);

		WP_Optimize()->include_template('cache/page-cache-rest-cache.php', false, $extract);
	}
	...
	private function get_curcy_cookies() {
		...
		if (!is_callable(array('WOOMULTI_CURRENCY_F_Data', 'get_ins'))) {
			error_log('WP-Optimize: Cache - WOOMULTI_CURRENCY_F_Data::get_ins() method not found'); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Using for debugging purpose
			return $cookies;
		} else {
			$curcy_settings = WOOMULTI_CURRENCY_F_Data::get_ins();
			if (!is_callable(array($curcy_settings, 'get_params'))) {
				error_log('WP-Optimize: Cache - WOOMULTI_CURRENCY_F_Data::get_params() method not found'); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Using for debugging purpose
				return $cookies;
			} else {
				...
				if (!is_callable(array($curcy_settings, 'get_auto_detect'))) {
					error_log('WP-Optimize: Cache - WOOMULTI_CURRENCY_F_Data::get_auto_detect() method not found'); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Using for debugging purpose
					return $cookies;
				} else {
					...
					if (1 === $auto_detect || 2 === $auto_detect) {
						...
						if (!is_callable(array($curcy_settings, 'get_geo_api'))) {
							error_log('WP-Optimize: Cache - WOOMULTI_CURRENCY_F_Data::get_geo_api() method not found'); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Using for debugging purpose
							return $cookies;
						} else {
	 ...
	 */
	private function cron_activate() {
		if ($this->is_local_gravatars_enabled()) {
	 ...
	 * @param array $config
	 */
	public function update_core_show_avatars($config) {
	 ...
	 * @param boolean $value
	 */
	public function update_gravatar_show_avatars($old_value, $value) {
	 ...
	 */
	private function host_gravatars_locally() {
	 ...
	 * @return bool
	 */
	private function is_local_gravatars_enabled() {
	 ...
	 * @return bool
	 */
	private function is_youtube_preview_enabled() {
