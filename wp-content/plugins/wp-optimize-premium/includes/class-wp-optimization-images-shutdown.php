...
class WP_Optimization_Images_Shutdown {
	...
	/**
	 * Set metadata which will returned in the output when script ending with failure.
	 *
	 * @param array $meta
	 */
	...
	public function shutdown() {
		...
		if (defined('WP_OPTIMIZE_UNUSED_IMAGES_LOG') && WP_OPTIMIZE_UNUSED_IMAGES_LOG) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_debug_backtrace -- Used for debugging purpose
	...
	/**
	 * Save post information (if there are any) in "bad posts" lists. "Bad" means that the post raise fatal error
	 * when we call do_shortcode(post_content) for it. If the post is "bad" then we don't call
	 * do_shortcode() for this post and parse images in the original post_content.
	 */
