...
class WP_Optimize_CLI_Command extends WP_CLI_Command {
	 ...
	 */
	public function __invoke($args, $assoc_args) { // phpcs:ignore PHPCompatibility.FunctionNameRestrictions.NewMagicMethods.__invokeFound -- Necessary
		...
		$commands = array(
			...
			// Minification
			// translators: %s is a command option
	...
	public function minify($params) {
			...
			if (!isset($params['feature'])) {
				...
				switch($params['feature']) {
					...
					case 'html':
						...
						break;
					default:
						// translators: %s is not a valid minify command option
		...
		if ('status' === $command) {
			...
			// translators: %s is minification status enabled/disabled
			...
			// translators: %s is javascript assets minification status ON/OFF
			...
			// translators: %s is css assets minification status ON/OFF
			...
			// translators: %s is html assets minification status ON/OFF
	 ...
	 *
	 * @param string $string Name of the color
	...
	private function colorize($string, $color) {
		...
		$token = $tokens[$color] ?? $tokens['bright'];
