...
class Feature
{
    ...
    function __construct()
    {
        ...
        $this->options = [
            [
                ...
                'description' => esc_html__('This product is a CitadelaWP Subscription.', 'citadela-directory')
            ],
            'items' => [
                'control' => 'input',
            ...
            'categories' => [
                ...
                'description' => esc_html__('Allow the customer to set catogories for CitadelaWP items.', 'citadela-directory')
            ],
            'locations' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_locations',
                'label' => esc_html__('Locations', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set locations for CitadelaWP items.', 'citadela-directory')
            ],
            'editor' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_editor',
                'label' => esc_html__('Content', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the content for CitadelaWP items.', 'citadela-directory')
            ],
            'thumbnail' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_thumbnail',
                'label' => esc_html__('Featured image', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the featured image for CitadelaWP items.', 'citadela-directory')
            ],
            'excerpt' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_excerpt',
                'label' => esc_html__('Excerpt', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the excerpt for CitadelaWP items.', 'citadela-directory')
            ],
            'page-attributes' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_page-attributes',
                'label' => esc_html__('Attributes', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the page attributes for CitadelaWP items.', 'citadela-directory')
            ],
            'comments' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_comments',
                'label' => esc_html__('Comments', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to manage comments for CitadelaWP items.', 'citadela-directory')
            ],
            'reviews' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_reviews',
                'label' => esc_html__('Reviews', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to manage reviews for CitadelaWP items.', 'citadela-directory')
            ],
            'general' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_general',
                'label' => esc_html__('General options', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the general options for CitadelaWP items.', 'citadela-directory')
            ],
            'address-location' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_address-location',
                'label' => esc_html__('Address and location', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the address and location for CitadelaWP items.', 'citadela-directory')
            ],
            'contact' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_contact',
                'label' => esc_html__('Contact', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the contact for CitadelaWP items.', 'citadela-directory')
            ],
            'opening_hours' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_opening_hours',
                'label' => esc_html__('Opening hours', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the opening hours for CitadelaWP items.', 'citadela-directory')
            ],
            'gallery' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_gallery',
                'label' => esc_html__('Images gallery', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the images gallery for CitadelaWP items.', 'citadela-directory')
            ],
            'featured_category' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_featured_category',
                'label' => esc_html__('Featured category', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the Featured Category for CitadelaWP items.', 'citadela-directory')
            ],
            'gpx_file_upload' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_gpx_file_upload',
                'label' => esc_html__('GPX Track', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the GPX track file for CitadelaWP items.', 'citadela-directory')
            ],
            'extension' => [
                'control' => 'checkbox',
                'id' => '_citadela_subscription_extension',
                'label' => esc_html__('Item Extension', 'citadela-directory'),
                'description' => esc_html__('Allow the customer to set the Item Extension options for CitadelaWP items.', 'citadela-directory')
            ]
        ];
                ...
                add_action('admin_init', function () {
                    ...
                    add_filter('woocommerce_product_data_tabs', function ($tabs) {
                        $tabs['citadela-subscription'] = [
                            'label' => esc_html__('CitadelaWP Subscription', 'citadela-directory'),
                        ...
                        add_action('pre_get_posts', function ($query) {
                            if ($screen = get_current_screen()) {
                                add_filter('views_' . $screen->id, function ($count) {
                                    unset($count['all']);
                                    unset($count['publish']);
                                    $count['trash'] = $this->restrict_trashed_count_to(
                                        isset($count['trash']) ? $count['trash'] : 0
                                    );
                                    return $count;
                                });
    ...
    public function restrict_trashed_count_to($trash_view) {
        global $wpdb;

        $user_id = get_current_user_id();

        $trashed_count = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->posts} 
         WHERE post_status = 'trash' 
         AND post_type = 'citadela-item' 
         AND post_author = %d",
            $user_id
        ));

        // Modify the trash count display
        if (isset($trash_view)) {
            $trash_view = preg_replace('/\((\d+)\)/', "($trashed_count)", $trash_view);
        }
    ...
    {
        $capability_map = [
            'general'           => 'citadela_item_option_general',
            'address-location'  => 'citadela_item_option_address_location',
            'contact'           => 'citadela_item_option_contact',
            'opening_hours'     => 'citadela_item_option_opening_hours',
            'gallery'           => 'citadela_item_option_images_gallery',
            'gpx_file_upload'   => 'citadela_item_option_gpx_track',
            'featured_category' => 'citadela_item_option_featured_category',
            'extension'         => 'citadela_item_option_custom_inputs',
        ];
        $permissions = [
            'general' => false,
        ...
        foreach ($capability_map as $key => $capability) {
            $permissions[$key] = current_user_can($capability);
        }
