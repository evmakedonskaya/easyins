...
class ClaimListing {
    ...
    static function run()
    {
    	self::$nonce = wp_create_nonce( 'ctdl-nonce' );
        ...
        add_action('admin_init', function() {
            if (self::enabled() && current_user_can('administrator')) {
                if (isset($_GET['citadela-claim-action']) && isset($_GET['_wpnonce'])) {
                    switch ($_GET['citadela-claim-action']) {
                        case 'approve':
                            self::approve($_GET['citadela-claim-item'], $_GET['_wpnonce']);
                            break;
                        case 'decline':
                            self::decline($_GET['citadela-claim-item'], $_GET['_wpnonce']);
                            break;
                    }
                            ...
                            if ($status === 'Unclaimed') {
                                esc_html_e('Unclaimed', 'citadela-directory');
                                    ...
                                    if ($user) {
                                        ...
                                        echo '<a href="' . esc_url( admin_url('edit.php?post_type=citadela-item&citadela-claim-action=approve&citadela-claim-item=' . $id . '&_wpnonce={' . self::$nonce. '}') ) . '" class="button">' . esc_html__('Approve', 'citadela-directory') . '</a>';
                                        echo '<a href="' . esc_url( admin_url('edit.php?post_type=citadela-item&citadela-claim-action=decline&citadela-claim-item=' . $id . '&_wpnonce={' . self::$nonce. '}') ) . '" class="button" onclick="return confirm(\'' . esc_html__('Are you sure you want to decline this claim?', 'citadela-directory') . '\')">' . esc_html__('Decline', 'citadela-directory') . '</a>';
                                    } else {
                                        esc_html_e('Unclaimed', 'citadela-directory');
                                    }
    ...
    static function approve($id, $n)
    {
	if (current_user_can('manage_options') && wp_verify_nonce( $n, 'ctdl-nonce' )) { 
