...
class Options 
{
	...
	protected static function plugins()
	{
		...
		foreach (self::$options['plugins']['citadelawp-directory']['special_pages'] as $option => $id) {
			if (empty($id)) {
		...
		if (!empty(self::$options['plugins']['citadelawp-pro']['special_pages'])) {
			foreach (self::$options['plugins']['citadelawp-pro']['special_pages'] as $option => $id) {
				if (empty($id)) {
					delete_option("citadela_{$option}");
				} else {
					update_option("citadela_{$option}", $id);
		...
		foreach (self::$options['plugins']['citadelawp-directory']['settings'] as $option => $value) {
			update_option($option, $value);
		}
		foreach (self::$options['plugins']['citadelawp-pro']['settings'] as $option => $value) {
			update_option($option, $value);
		}
			...
			foreach (self::$options['woocommerce'] as $option) {
				if( isset( $excluded_options['woocommerce'] ) && ! empty( $excluded_options['woocommerce'] ) ) {
					if( ! in_array($option[1], $excluded_options['woocommerce']) ) {
                        update_option( $option[1], self::fix_unserialize( $option[2] ) );
					}
				}else{
					update_option( $option[1], self::fix_unserialize( $option[2] ) );
				}
    ...
    protected static function fix_unserialize($data) {
        while (is_string($data) && ($temp = @unserialize($data)) !== false) {
            $data = $temp;
        }
        return $data;
    }
