...
function citadela_handle_ait_plugins_support() {
    $ait_plugins = array_filter(get_option('active_plugins', []), function ($plugin) {
        $allowed_plugins = [
            'ait-easydocs/ait-easydocs.php',
            'ait-blocks/ait-blocks.php',
            'ait-templates/ait-templates.php',
        ];
        // Skip this specific plugin
        if ( in_array( $plugin, $allowed_plugins, true ) ) {
            return false;
        }

        return (
            (substr($plugin, 0, 4) === 'ait-') ||
            (substr($plugin, 0, 9) === 'citadela-')
        );
    });
    ...
    add_action( 'admin_notices', function() use ($ait_plugins) {
        ...
        printf(
            ...
            sprintf(
                ...
                esc_html__( "We've noticed that you're running CitadelaWP incompatible plugins (%s) so we deactivated them.", 'citadela' ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
