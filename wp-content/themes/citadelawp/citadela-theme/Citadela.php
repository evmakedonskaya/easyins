    ...
    $citadela[33] = function () {
        class Citadela
        {
            ...
            static function init()
            {
                ...
                Citadela::$zone = explode('/', preg_replace('|https?://|', '', get_option('siteurl')))[0];
                Citadela::$key = get_option('citadela_api_key', '');
                Citadela::$package = isset(Citadela::$package) ? Citadela::$package : '';
                Citadela::$package_envato = (Citadela::$package === 'themeforest' && defined('CITADELAWP_DIRECTORY_PLUGIN') && !defined('CITADELAWP_THEME') && !defined('CITADELAWP_PRO_PLUGIN')) ? 'codecanyon' : 'themeforest';
                $lb = strpos(Citadela::$zone, hex2bin('6c6f63616c686f7374')) === 0 || strpos(Citadela::$zone, hex2bin('3132372e302e3031')) === 0;
                if ($lb) {
                    $response['code'] = 200;
                } else {
                ...
                if (Citadela::$trial && (defined('CITADELAWP_DIRECTORY_PLUGIN') || defined('CITADELAWP_PRO_PLUGIN'))) {
                    $box = '<div class="citadela-notice-trial">' . sprintf(
                        /*translators: 1. Start html anchor tag, 2. End html anchor tag  */ 
                        __('This website is running trial version of CitadelaWP products. Please %1$spurchase a license%2$s to activate the website.', 'citadela'),
                        '<a href="https://www.ait-themes.club/">',
                        '</a>'
                    ) . '</div>';
                ...
                if (!Citadela::$allowed && (defined('CITADELAWP_DIRECTORY_PLUGIN') || defined('CITADELAWP_PRO_PLUGIN'))) {
                    if (is_super_admin()) {
                ...
                add_filter('pre_set_site_transient_update_themes', function ($value) {
                    return Citadela::checkUpdates($value, 'themes');
                });
 
                add_action('admin_init', function () {
                    register_setting('citadela-api', 'citadela_api_key');
                });
                if (defined('CITADELAWP_THEME')) {
                    add_action('citadela_updater_options', function () use ($lb) {
                        ?>
                        <form method="post" action="options.php">
                            ...
                            <?php if (!in_array(Citadela::$package, ['themeforest', 'mojo', 'themely'])) { ?>
                                <div class="setting-part setting-domain">
                                    ...
                                    <div class="setting">
                                        <input type="text" class="regular-text" name="citadela_domain" id="citadela_domain" value="<?php echo esc_attr(Citadela::$zone); ?>" readonly="true">
                            ...
                            <div class="setting-part setting-key">
                                ...
                                <div class="setting">
                                    <input type="text" class="regular-text" name="citadela_api_key" id="citadela_api_key" value="<?php echo $lb ? esc_attr__('The API key is not needed.', 'citadela') : esc_attr(Citadela::$key); ?>"<?php echo $lb ? esc_html(' readonly="true"') : ''; ?>>
                ...
                add_action('admin_init', function () {
                    if (current_user_can('install_plugins') && !defined('CITADELAWP_PRO_PLUGIN')) {
                        if (!(isset($_GET['page']) && $_GET['page'] == 'citadela-settings' && !defined('CITADELAWP_PRO_PLUGIN') && !defined('CITADELAWP_DIRECTORY_PLUGIN'))) {
                            add_action('admin_notices', function () { ?>
                                <div id="citadela-installation" class="notice">
                                    <p class="notice-info"><?php _e('Extend the functionality of the CitadelaWP theme with the premium CitadelaWP Pro and CitadelaWP Listing plugins, which include ready-to-use CitadelaWP layouts.', 'citadela'); ?></p>
                                    <p class="notice-buttons">
                                        <a class="citadela-installation" href="<?php echo admin_url('admin-ajax.php?action=citadela-installation'); ?>"><?php _e('Install CitadelaWP plugins', 'citadela'); ?></a> <strong class="citadela-installation-notice citadela-installation-progress"><?php _e('Installing...', 'citadela'); ?></strong> <strong class="citadela-installation-notice citadela-installation-error"><?php _e('Error with installing', 'citadela'); ?></strong>
                                    </p>
                                </div>
                                <div class="citadela-installation-notice citadela-installation-success notice notice-success is-dismissible">
                                    <p>
                                        <?php echo Citadela::$business ? _e('CitadelaWP Pro plugin installed.', 'citadela') : _e('Premium CitadelaWP plugins installed.', 'citadela'); ?>
                                    </p>
                                </div>
                                <?php
                            });
            ...
            static function theme()
            {
                if (!isset(Citadela::$cache['theme'])) {
                    ...
                    if ($active->get('Name') === 'CitadelaWP' && $active->get('Author') === 'AitThemes') {
                    ...
                    if ($available->exists() && $available->get('Name') === 'CitadelaWP' && $available->get('Author') === 'AitThemes') {
                        return Citadela::$cache['theme'] = [
                            'active' => false,
                            'available' => true,
            ...
            static function plugins()
            {
                if (!isset(Citadela::$cache['plugins'])) {
                    ...
                    foreach (get_plugins() as $path => $plugin) {
                        ...
                        $plugin['free'] = $name === 'citadelawp-directory-lite';
                        Citadela::$cache['plugins'][strpos($path, 'citadelawp-') === 0 ? 'citadela' : 'others'][$name] = $plugin;
                    }
                }
            ...
            static function checkUpdates($value, $type)
            {
                if (!isset(Citadela::$cache['versionsLatest'])) {
                    Citadela::$cache['versionsLatest'] = json_decode(wp_remote_retrieve_body(wp_remote_post(Citadela::$url . '/core/products/latest-versions', [
                        'body' => json_encode(array_merge(Citadela::theme()['available'] ? ['citadelawp'] : [], array_keys(Citadela::plugins()['citadela'])))
                    ])), true);
                }
                switch ($type) {
                    case 'themes':
                        if (Citadela::theme()['available'] && (version_compare(Citadela::$cache['versionsLatest']['citadelawp'], Citadela::theme()['version']) > 0)) {
                            $data['theme'] = 'citadelawp';
                            $data['new_version'] = Citadela::$cache['versionsLatest']['citadelawp'];
                            $data['package'] = Citadela::$url . '/core/products/file?' . http_build_query(['package' => Citadela::$package, 'domain' => Citadela::$zone, 'key' => Citadela::$key, 'name' => 'citadelawp']);
                            $data['url'] = Citadela::$url . '/themes/changelog/citadelawp';
                            $data['tested'] = '6.6.1';
                            $value->response['citadelawp'] = $data;
                        }
                        break;
                    case 'plugins':
                        foreach (Citadela::plugins()['citadela'] as $name => $plugin) {
                            if (isset(Citadela::$cache['versionsLatest'][$name]) && version_compare(Citadela::$cache['versionsLatest'][$name], $plugin['Version']) > 0) {
                                ...
                                $data->package = Citadela::$url . '/core/products/file?' . http_build_query(['package' => Citadela::$package, 'domain' => Citadela::$zone, 'key' => Citadela::$key, 'name' => $name]);
                                $data->tested = '6.6.1';
            ...
            static function getResponseMessage($response)
            {
                ...
                if (defined('CITADELAWP_THEME')) {
                    $url = '<a href="' . esc_url(admin_url('themes.php?page=citadela-settings')) . '">(' . esc_html__('Admin &rarr; CitadelaWP Theme', 'citadela') . ')</a>';
                } else if (Citadela::$package_envato === 'codecanyon') {
                    $url = '<a href="' . esc_url(admin_url('admin.php?page=citadela-directory-settings&citadela_directory_tab=plugin_activation')) . '">(' . esc_html__('Admin &rarr; CitadelaWP Listing &rarr; Plugin Activation', 'citadela') . ')</a>';
                } else {
                    $url = '<a href="' . esc_url(admin_url('options-general.php?page=citadela-api')) . '">(' . esc_html__('Admin &rarr; Settings &rarr; CitadelaWP','citadela') . ')</a>';
                }
                if ($response['code'] == 401) {
                    if (isset($response['body']['former'])) {
                        $message['title'] = esc_html__('Expired license', 'citadela');
                        $message['message'] = sprintf(
                            /*translators: 1. Start html anchor tag, 2. End html anchor tag */
                            __('Your membership or license has expired. You can %1$srenew it here%2$s.', 'citadela'), 
            ...
            static function downloadProduct($name)
            {
                $products = array_merge(
                    Citadela::theme()['available'] ? ['citadela'] : [],
                    array_keys(Citadela::plugins()['citadela'])
                );
                Citadela::verifyAccount($products, true);
                return download_url(Citadela::$url . '/core/products/file?' . http_build_query(['package' => Citadela::$package, 'domain' => Citadela::$zone, 'key' => Citadela::$key, 'name' => $name]));
            }
            static function checkPlugin($name)
            {
                $citadela = strpos($name, 'citadelawp-') === 0;
            ...
            static function installAndActivatePlugin($name)
            {
                ...
                if (strpos($name, 'citadelawp-') === 0) {
                    $api = new StdClass;
                    $api->download_link = Citadela::$url . '/core/products/file?' . http_build_query(['package' => Citadela::$package, 'domain' => Citadela::$zone, 'key' => Citadela::$key, 'name' => $name]);
                } else {
